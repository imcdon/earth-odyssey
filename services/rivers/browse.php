 ...
 */
function mover_row(array $m, string $code, string $win, string $units): string
{
    ...
    return '<li class="river-mover"><div class="river-mover-main"><a href="' . htmlspecialchars($href) . '">' . htmlspecialchars($m['display_name']) . '</a>'
        . '<span class="river-mover-meta">' . htmlspecialchars((string) $m['county']) . ' &middot; ' . htmlspecialchars(river_time_ago($m['observed_now'] . ' UTC')) . '</span></div>'
                ...
                <div class="river-movers-cols">
                    <?php foreach (['up' => 'Rising', 'down' => 'Dropping'] as $dir => $dirLabel): ?>
                        <div class="river-movers-col is-<?= $dir ?>">
                            ...
                            <?php else: ?>
                                <ol class="river-movers-list"><?php foreach ($lists[$dir] as $m) echo mover_row($m, $code, $win, $units); ?></ol>
                            <?php endif; ?>
                        </div>
                    <?php endforeach; ?>
                </div>
